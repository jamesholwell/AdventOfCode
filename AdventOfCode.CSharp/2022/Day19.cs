...
public class Day19(string? input = null, ITestOutputHelper? outputHelper = null)
    ...
    private readonly struct Blueprint {
        ...
        public Blueprint(string s) {
            ...
            OreRobotLimit = Math.Max(Math.Max(Math.Max(OreRobotOreCost, ClayRobotOreCost), ObsidianRobotOreCost),
                GeodeRobotOreCost);
        }
    ...
    private readonly struct Scenario(
        ...
        int obsidianRobots,
        int timeLeft,
        int lifetimeGeodesCracked) {
        public readonly Blueprint Blueprint = blueprint;

        public readonly int Ores = ores;

        public readonly int Clays = clays;

        public readonly int Obsidians = obsidians;

        public readonly int OreRobots = oreRobots;

        public readonly int ClayRobots = clayRobots;

        public readonly int ObsidianRobots = obsidianRobots;

        public readonly int TimeLeft = timeLeft;

        public readonly int LifetimeGeodesCracked = lifetimeGeodesCracked;
        ...
        public override int GetHashCode() {
            return HashCode.Combine(Ores, Clays, Obsidians, OreRobots, ClayRobots, ObsidianRobots, TimeLeft,
                LifetimeGeodesCracked);
        }
    }
    ...
    private IDictionary<Blueprint, int> Solve(Blueprint[] blueprints, int timeLeft) {
        ...
        while (currentScenarios.Any()) {
            ...
            Trace.WriteLine($"Pass {++i} with {passOperations:N0} scenarios to evaluate");
            ...
            currentScenarios.AsParallel().ForAll(scenario => {
                ...
                if (scenario.ObsidianRobots > 0) {
                    var timeUntilGeodeRobotReady = 1 + Math.Max(
                        Math.Max(0,
                            (blueprint.GeodeRobotOreCost - scenario.Ores + scenario.OreRobots - 1) /
                            scenario.OreRobots),
                        Math.Max(0,
                            (blueprint.GeodeRobotObsidianCost - scenario.Obsidians + scenario.ObsidianRobots - 1) /
                            scenario.ObsidianRobots)
                    );
                ...
                if (scenario.ClayRobots > 0 && scenario.ObsidianRobots < blueprint.GeodeRobotObsidianCost) {
                    var timeUntilObsidianRobotReady = 1 + Math.Max(
                        Math.Max(0,
                            (blueprint.ObsidianRobotOreCost - scenario.Ores + scenario.OreRobots - 1) /
                            scenario.OreRobots),
                        Math.Max(0,
                            (blueprint.ObsidianRobotClayCost - scenario.Clays + scenario.ClayRobots - 1) /
                            scenario.ClayRobots)
                    );
                ...
                if (scenario.ClayRobots < blueprint.ObsidianRobotClayCost) {
                    var timeUntilClayRobotReady = 1 + Math.Max(0,
                        (blueprint.ClayRobotOreCost - scenario.Ores + scenario.OreRobots - 1) / scenario.OreRobots);
                ...
                if (scenario.OreRobots < blueprint.OreRobotLimit) {
                    var timeUntilOreRobotReady = 1 + Math.Max(0,
                        (blueprint.OreRobotOreCost - scenario.Ores + scenario.OreRobots - 1) / scenario.OreRobots);
            ...
            Trace.WriteLine($"... done exploring, building next pass");
            ...
            timer.Stop();
            Trace.WriteLine(timer.ElapsedMilliseconds < 1
        ...
        Trace.WriteLine();
        Trace.WriteLine($"Total of {ops:N0} scenarios inspected");
        Trace.WriteLine();

        foreach (var blueprint in lowerBounds) {
            Trace.WriteLine($"Blueprint {blueprint.Key.Id} produces at most {blueprint.Value} geodes");
        }
...
";
    ...
    public void SolvesPartOneExample() {
        var actual = new Day19(ExampleInput, Trace).SolvePartOne();
        Assert.Equal(33, actual);
    }

    [Fact]
    public void SolvesPartTwoExample() {
        var actual = new Day19(ExampleInput, Trace).SolvePartTwo();
