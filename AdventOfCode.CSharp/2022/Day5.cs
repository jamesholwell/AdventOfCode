...
public class Day5(string? input = null, ITestOutputHelper? outputHelper = null) : Solver<string>(input, outputHelper) {
    protected override string SolvePartOne() => Solve(false);

    protected override string SolvePartTwo() => Solve(true);

    private string Solve(bool isCrateMover9001) {
    ...
    private static Stack<char>[] ParseCrates(string s) {
        var lines = s.Split("\n");
        var stacks = Enumerable.Range(0, (1 + lines[0].Length) / 4).Select(_ => new Stack<char>()).ToArray();
